...
names(base_dirs) <- regions

fixed_form <- ~ Tipo * ns(frac_area, df = 3)

frac_seq <- seq(0.01, 1.00, length.out = 200)
...
for (region_name in regions) {
  ...
  for (f in model_files) {
    ...
    run_id <- as.integer(
      gsub(".*model_run_([0-9]+)_.*", "\\1", basename(f))
    )
    ...
    newdata$pred <- as.numeric(
      X[, common, drop = FALSE] %*% beta[common]
    )
    
    newdata$region <- region_name
    newdata$response <- response_var
    newdata$run <- run_id
    
    curves_region[[as.character(run_id)]] <- as.data.table(newdata)
    
    coefs_region[[as.character(run_id)]] <- data.table(
      region = region_name,
      response = response_var,
      run = run_id,
      term = names(beta),
  ...
  curves_region <- rbindlist(curves_region)
  coefs_region <- rbindlist(coefs_region)
  
  curves_summary_region <- curves_region[
    ,
    .(
    ...
    by = .(region, response, frac_area, Tipo)
  ]
  
  coefs_summary_region <- coefs_region[
    ,
    .(
      median_beta = median(estimate, na.rm = TRUE),
      lower_beta = quantile(estimate, 0.025, na.rm = TRUE),
      upper_beta = quantile(estimate, 0.975, na.rm = TRUE),
      mean_beta = mean(estimate, na.rm = TRUE),
      sd_beta = sd(estimate, na.rm = TRUE)
    ),
    by = .(region, response, term)
  ]
  
  fwrite(
    curves_region,
    file.path(out_dir, "tables", paste0("fixed_curves_all_runs_", region_name, ".csv"))
  )
  
  fwrite(
    curves_summary_region,
    file.path(out_dir, "tables", paste0("fixed_curves_ensemble_summary_", region_name, ".csv"))
  )
  
  fwrite(
    coefs_region,
    file.path(out_dir, "tables", paste0("fixed_coefficients_all_runs_", region_name, ".csv"))
  )
  
  fwrite(
    coefs_summary_region,
    file.path(out_dir, "tables", paste0("fixed_coefficients_ensemble_summary_", region_name, ".csv"))
  )
  
  all_curves[[region_name]] <- curves_summary_region
  all_coefs[[region_name]] <- coefs_summary_region
}

curves_summary_all <- rbindlist(all_curves)
coefs_summary_all <- rbindlist(all_coefs)

fwrite(
  curves_summary_all,
  paste0("fixed_curves_ensemble_summary_all_regions_", response_var, ".csv")
)

fwrite(
  coefs_summary_all,
  paste0("fixed_coefficients_ensemble_summary_all_regions_", response_var, ".csv")
)
...
make_p_adj <- function(dat, response_var, show_legend = FALSE, first_col = FALSE) {
  ...
  dat_adj <- dat %>% 
    filter(contrast_type == "Spatially adjusted")
  
  yrng <- range(c(dat_adj$lower, dat_adj$upper, dat_adj$median), na.rm = TRUE)
  dy <- diff(yrng)
  
  tick_top <- yrng[2] + 0.095 * dy
  tick_half <- 0.004
  
  dat_sig <- dat_adj %>%
    mutate(
      sig_col = ifelse(significant, as.character(stage), "Not significant"),
      y_tick = case_when(
        stage == "SF[e]" ~ tick_top,
        stage == "SF[i]" ~ tick_top + 0.035 * dy,
        stage == "SF[a]" ~ tick_top + 0.070 * dy
      ),
      tick_half <- 0.004,
      x_center = pmin(pmax(frac_plot, tick_half), 1 - tick_half),
      x_start  = x_center - tick_half,
      x_end    = x_center + tick_half
    )
  ...
  ggplot(
    ...
    coord_cartesian(ylim = yrng, clip = "off") +
    
    scale_color_manual(
      values = c(
        cols_by_response[[response_var]],
        "Not significant" = "grey95"
      ),
      breaks = c("SF[e]", "SF[i]", "SF[a]"),
    ...
    theme(
      ...
      legend.box.margin = margin(t = 2, r = 0, b = 8, l = 0),
      plot.margin = margin(t = 8, r = 6, b = 10, l = 4),
...
make_p_land <- function(dat, response_var, show_legend = FALSE, first_col = FALSE) {
  ...
  dat_land <- dat %>% 
    filter(contrast_type == "Current landscape")
  
  yrng <- range(c(dat_land$median), na.rm = TRUE)
  dy <- diff(yrng)
  ...
  ggplot(
    ...
    geom_linerange(
      ...
      aes(
        x = frac_bin,
        ymin = y_tick - 0.008 * dy,
        ymax = y_tick + 0.008 * dy,
        color = sig_col
      ),
      linewidth = 1.15,
      inherit.aes = FALSE,
...
make_p_year <- function(dat, response_var, first_col = FALSE) {
  ...
  ggplot(dat, aes(x = Ano, y = median)) +
    ...
    geom_ribbon(
      ...
      fill = color_year[[response_var]],
      alpha = 0.25
    ) +
    
    geom_line(
      linewidth = 0.95,
      color = color_year[[response_var]]
    ) +
...
contrast_type_select <- "Spatially adjusted"

cover_values <- c(0.25, 0.50, 0.75)
...
contrast_labels <- c(
  PF_minus_SF_early = "PF - SF[e]",
  PF_minus_SF_intermediate = "PF - SF[i]",
  PF_minus_SF_advanced = "PF - SF[a]"
)
...
calc_stability_one <- function(region_name,
                               response_var,
                               contrast_col,
                               cover_value,
                               n_perm = 200) {
  ...
  file_contrasts <- file.path(
    out_dir,
    "tables",
    "contrasts_ensemble_all_runs.csv"
  )
  
  if (!file.exists(file_contrasts)) {
    stop("File not found: ", file_contrasts)
  }
  
  contr <- fread(file_contrasts)
  
  tmp0 <- contr %>%
    filter(contrast_type == contrast_type_select)
  
  tmp <- tmp0 %>%
  mutate(
    cover_diff = abs(frac_area - cover_value)
  ) %>%
  group_by(run) %>%
  slice_min(
    order_by = cover_diff,
  ...
  ungroup() %>%
  select(run, value = all_of(contrast_col)) %>%
  filter(!is.na(value))
  
  runs_available <- sort(unique(tmp$run))
  
  if (length(runs_available) < 3) {
    stop(
      "Too few runs available for ",
      response_var, " | ",
      contrast_col, " | cover ",
      cover_value
    )
  ...
  for (p in seq_len(n_perm)) {
    ...
    run_order <- sample(runs_available)
    
    for (n in 2:length(run_order)) {
      
      sel_runs <- run_order[1:n]
      
      est <- tmp %>%
        filter(run %in% sel_runs) %>%
        summarise(
          median_est = median(value, na.rm = TRUE)
        ) %>%
        pull(median_est)
      
      stab_list[[length(stab_list) + 1]] <- data.frame(
        response = response_var,
        ...
        n_runs = n,
        estimate = est,
        contrast = contrast_labels[[contrast_col]],
        cover_value = cover_value,
        cover_label = paste0(round(cover_value * 100), "% cover")
      )
    }
  }
...
stab_df <- stab_df %>%
  ...
  mutate(
    final_estimate = median(
      estimate[n_runs == max(n_runs)],
      na.rm = TRUE
    ),
    estimate_centered =
...
stab_summary <- stab_df %>%
  ...
  summarise(
    median = median(estimate_centered, na.rm = TRUE),
    lower = quantile(estimate_centered, 0.025, na.rm = TRUE),
    upper = quantile(estimate_centered, 0.975, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    response_label = factor(
      response_label,
      levels = response_labels[response_vars]
    ),
    contrast = factor(
      contrast,
      levels = contrast_labels[contrast_cols]
    ),
    cover_label = factor(
      cover_label,
      levels = paste0(round(cover_values * 100), "% cover")
    )
  )


out_table_dir <- file.path("fig",  "stability",  "tables")

dir.create(out_table_dir, recursive = TRUE, showWarnings = FALSE)

fwrite(stab_df,
  file.path(out_table_dir,
    paste0("ensemble_stability_all_permutations_all_variables_", region_name, ".csv")))

fwrite( stab_summary,
  file.path(out_table_dir,
    paste0("ensemble_stability_summary_all_variables_", region_name, ".csv")))

format_y_stability <- function(x) {
  ifelse(abs(x) < 0.1, sprintf("%.2f", x), as.character(x))
}

p_stability_all <- ggplot(
  stab_summary,
  aes(x = n_runs, y = median, color = response, fill = response)
) +
  geom_ribbon(
    aes(ymin = lower, ymax = upper),
    alpha = 0.16,
  ...
  geom_point(size = 1.1) +
  facet_grid(
    response_label + contrast ~ cover_label,
    scales = "free_y",
    labeller = labeller(
      contrast = label_parsed)) +
  scale_y_continuous(
  labels = format_y_stability) +
  scale_color_manual(values = color_year, guide = "none") +
  scale_fill_manual(values = color_year, guide = "none") +
  labs(
    x = "Number of ensemble repetitions",
    y = "Deviation from the final ensemble estimate (30 runs)",
    title = paste0("Stability of ensemble estimates - ", region_name)
  ) +
...
out_fig_dir <- file.path("fig", "stability")
dir.create(out_fig_dir, recursive = TRUE, showWarnings = FALSE)

ggsave(
  file.path(out_fig_dir,
    paste0("ensemble_stability_all_variables_", region_name, ".png")  ),
  p_stability_all,  width = 10,  height = 12.5,  dpi = 600,  bg = "white")

ggsave(
  file.path(out_fig_dir,
    paste0("ensemble_stability_all_variables_", region_name, ".pdf")),
  p_stability_all,  width = 10,  height = 12.5,  bg = "white")
...
make_p_land <- function(dat, response_var, show_legend = FALSE, first_col = FALSE) {
  ...
  yrng <- range(c(dat_land$lower, dat_land$upper, dat_land$median, 0), na.rm = TRUE)
  dy <- diff(yrng)
...
make_p_spatial <- function(dat, response_var, first_col = FALSE) {
  ...
  ggplot(dat, aes(x = Lon, y = Lat, z = spatial_median)) +
    ...
    theme(
      ...
      legend.title = element_text(size = 9, face = "bold"),
      legend.text = element_text(size = 8.5),
      legend.box.margin = margin(t = -6, r = 0, b = 2, l = 0),
      legend.margin = margin(t = -6, r = 0, b = 2, l = 0),
      plot.margin = margin(t = -8, r = 2, b = 8, l = 2),
      panel.grid.minor = element_blank())
...
fig_region <- 
  ...
  wrap_plots(p_year_list, nrow = 1) /
  wrap_plots(p_spatial_list, nrow = 1,  widths = c(1, 1, 1)) +
  plot_layout(heights = c(1, 1, 0.9, 0.8)) +
  plot_annotation(
    title = region_name,
    theme = theme(
      plot.title = element_text(
